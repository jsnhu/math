...
function wireNumpad(onInput) {
  // Pointer events for the on-screen numpad keys only
  document.querySelectorAll('.key').forEach(key => {
    key.addEventListener('pointerdown', e => {
      e.preventDefault(); // prevent the mouse-event fallback (avoids double-fire)
  ...
  document.addEventListener('keydown', e => {
    if (e.target.tagName === 'INPUT') return; // don't intercept settings fields

    // Digit row and numpad digits
    if (e.key >= '0' && e.key <= '9') {
      e.preventDefault();
      onInput(e.key);
      return;
